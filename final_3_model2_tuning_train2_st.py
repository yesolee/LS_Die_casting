...
#     # K-Fold 교차 검증
#     for train_index, valid_index in kf.split(X_train2, y_train2):
#         X_train_fold, X_valid_fold = X_train2.iloc[train_index], X_train2.iloc[valid_index]
#         y_train_fold, y_valid_fold = y_train2.iloc[train_index], y_train2.iloc[valid_index]
...
# Best parameters: {'n_estimators': 250, 'learning_rate': 0.10397331142773888, 'depth': 9, 'l2_leaf_reg': 6}
# Best geometric mean score: 0.9509593854303565
best_params = {'n_estimators': 250, 'learning_rate': 0.10397331142773888, 'depth': 9, 'l2_leaf_reg': 6}
...
X_train2_under, y_train2_under = sampler_under.fit_resample(X_train2, y_train2)
best_model.fit(X_train2_under, y_train2_under)
...
# Test predictions: 0.17045454545454544
# Test recall: 0.8823529411764706
# Test f1: 0.2857142857142857
# Test roc_auc: 0.9941208335988784
# Test g_mean: 0.930003566862189

# from catboost import Pool, CatBoostClassifier, plot_tree
# import matplotlib.pyplot as plt

# # 이미 학습된 모델 사용
# # best_model.fit(X_train2_under, y_train2_under)

# # 학습에 사용된 데이터를 Pool로 감싸기
# pool = Pool(X_train2_under, y_train2_under)

# # 트리 시각화
# plot_tree(best_model, pool, tree_idx=0)  # tree_idx는 시각화할 트리 인덱스
# plt.show()
...
feature_names = X_train2_under.columns  # 학습에 사용된 X_train1_under의 컬럼명 사용
...
# PDP 시각화
n_features = X_train2_under.shape[1]  # 학습에 사용된 데이터 사용
n_rows = 12  # 12행
n_cols = 5   # 5열
    ...
    for j in range(n_cols):
        ...
        if feature_index < n_features:  # 변수 수 범위 체크
            PartialDependenceDisplay.from_estimator(
                best_model,
                X_train2_under,  # 학습에 사용된 데이터로 변경
...
shap_values = explainer.shap_values(X_train2_under)

# 1. SHAP summary plot: 전체 변수 중요도와 분포를 시각화
shap.summary_plot(shap_values, X_train2_under)
