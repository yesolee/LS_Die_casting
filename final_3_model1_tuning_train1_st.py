...
#     # K-Fold 교차 검증
#     for train_index, valid_index in kf.split(X_train1, y_train1):
#         X_train_fold, X_valid_fold = X_train1.iloc[train_index], X_train1.iloc[valid_index]
#         y_train_fold, y_valid_fold = y_train1.iloc[train_index], y_train1.iloc[valid_index]
...
# Best parameters: {'n_estimators': 475, 'learning_rate': 0.043675045218176826, 'max_depth': 11, 'lambda_l1': 0.08380613094504535, 'lambda_l2': 1.5540690236820405, 'subsample': 0.8023710837830162, 'colsample_bytree': 0.5004774666324261}
# Best geometric mean score: 0.980348183305004
best_params = {'n_estimators': 475, 'learning_rate': 0.043675045218176826, 'max_depth': 11, 'lambda_l1': 0.08380613094504535, 'lambda_l2': 1.5540690236820405, 'subsample': 0.8023710837830162, 'colsample_bytree': 0.5004774666324261}
...
X_train1_under, y_train1_under = sampler_under.fit_resample(X_train1, y_train1)
best_model.fit(X_train1_under, y_train1_under)
...
# Test predictions: 0.022788203753351208
# Test recall: 1.0
# Test f1: 0.04456094364351245
# Test roc_auc: 0.9977853546619081
# Test g_mean: 0.8958493430838647

import lightgbm as lgb
fig, ax = plt.subplots(figsize=(30, 20), dpi=300)  # DPI 값을 조정하여 해상도를 높일 수 있습니다.
...
feature_names = X_train1_under.columns  # 학습에 사용된 X_train1_under의 컬럼명 사용
...
# PDP 시각화
n_features = X_train1_under.shape[1]  # 학습에 사용된 데이터 사용
n_rows = 12  # 12행
n_cols = 5   # 5열
    ...
    for j in range(n_cols):
        ...
        if feature_index < n_features:  # 변수 수 범위 체크
            PartialDependenceDisplay.from_estimator(
                best_model,
                X_train1_under,  # 학습에 사용된 데이터로 변경
...
shap_values = explainer.shap_values(X_train1_under)

# 1. SHAP summary plot: 전체 변수 중요도와 분포를 시각화
shap.summary_plot(shap_values, X_train1_under)
